...
export default function FlowerCard({ data, index, isDragging, deltaX }) {
    ...
    const handleClick = (e) => {
        if (isDragging || currentPage.element) return
    ...
    return (
        <div ref={wrapperRef} className={`flower__card__wrapper card__wrapper-${data.slug}`} >
            <img ref={imgRef} onTouchEnd={handleClick} onClick={handleClick} className={`flower__card__image card__image-${data.slug}`} src={"/images/flowers/thumbnails/" + data.thumbnail} alt="" style={imageStyle} />

            <div ref={titleRef} className={`flower__card__title card__title-${data.slug}`} style={titleStyle} onTouchEnd={handleClick} onClick={handleClick}>
